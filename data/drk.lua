...
--[[ AltF9-AltF12 keybinds ]]
	...
	send_command('bind !f10 gs c toggle DarkSealMode')
...
--[[ CtrlF9-CtrlF12 keybinds ]]
	send_command('bind ^f9 gs c toggle AutoWSMode')
	send_command('bind ^f10 gs c cycle Stance')
	send_command('bind ^f11 gs c toggle AutoStunMode')
	send_command('bind ^f12 gs c toggle AutoFoodMode')
	send_command('bind ^` input /ma "Dark Seal" <me>')
	send_command('bind ^q input /ma "Nether Void" <me>')
--[[ WindowsF9-WindowsF12 keybinds ]]
	...
	send_command('bind @` input /ma "Endark II" <me>')
...
--------------------------
function job_setup()
	state.mainWeapon = M{'None', 'Apocalypse', 'Caladbolg', 'Ragnarok', 'Anguta'}
	...
	init_job_states({
		"AutoRuneMode","AutoWSMode","AutoShadowMode",
		"AutoFoodMode","AutoStunMode","AutoBuffMode"
		},{
		"Weapons","WeaponskillMode","Stance"
	})
...
--[[ Job Specific Buff handling ]]
	...
	elseif not buffactive['Last Resort'] then 
...
function select_default_macro_book()
	...
	if player.sub_job == 'SAM' and player.equipment.main == 'Ragnarok' then
		set_macro_page(2, 2)
